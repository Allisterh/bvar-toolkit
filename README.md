...
functionized drivers are tested the same way against the original scripts in full. The
library takes the lower Cholesky factor throughout. Where the legacy code factors a dense
matrix with the upper one, as for `bvar.ml.llike_ma` and `bvar.ml.lniwpdf`, the test gives the
legacy code the same substitution, because the two factors of a dense matrix can differ in
the last bits.
