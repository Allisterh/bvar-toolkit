%% GOLDEN-RUN VARIANT PATCH (2026-09-17): identical to legacy/main_AD_VAR.m except that
%% the call to plotforecast and the commented data plot at the end are replaced by a
%% numeric capture, which prints the forecasts and their sensitivities and saves them
%% to golden_ad_var.mat. The sensitivity block above it is untouched.
...
fprintf('forecast means at horizon 1:  %.6g %.6g %.6g\n', means(1,:));
fprintf('forecast means at horizon 20: %.6g %.6g %.6g\n', means(end,:));
fprintf('84%% quantiles at horizon 1:   %.6g %.6g %.6g\n', Q_upper(1,:));
fprintf('16%% quantiles at horizon 1:   %.6g %.6g %.6g\n', Q_lower(1,:));
fprintf('unemployment forecast mean at horizon 1, derivatives wrt kappa1, kappa2, kappa3: %.6g %.6g %.6g\n', squeeze(Ym.d(1,1,:)));
fprintf('norm(bm): %.6g\n', norm(bm));
Ym_d = Ym.d;
save('golden_ad_var.mat','means','Q_upper','Q_lower','Ym_d','bm','d_Qupper','d_QLower');
