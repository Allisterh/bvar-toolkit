% bvar.ml.mltvpsv_tvpsv - log marginal likelihood of the TVP-SV model of Chan and
% Eisenstat (2018) by importance sampling. The importance density of each variance is
% inverse-gamma and that of each initial state Gaussian, fitted to the posterior draws;
% the states and log-volatilities are integrated out by bvar.ml.intlike_tvpsv. The M
...
%
%   [lml, lmlstd, out] = bvar.ml.mltvpsv_tvpsv(Y, store_Sigtheta, store_Sigh, ...
%       store_h0, store_theta0, prior, bigX, M)
%
%   Y     : T*n x 1 data, stacked by period
%   store_*: posterior draws, one row per draw
%   prior : log prior density, prior(Sigtheta, Sigh, theta0, h0)
...
function [ml, mlstd, out] = mltvpsv_tvpsv(Y,store_Sigtheta,store_Sigh,store_h0,store_theta0,...
    prior,bigX,M)
M = 20*ceil(M/20);
n = size(store_Sigh,2);
...
for loop = 1:M
    ...
    llike = bvar.ml.intlike_tvpsv(Y,Sigtheta,Sigh,bigX,h0,theta0);
