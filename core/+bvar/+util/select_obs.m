% bvar.util.select_obs - the selection matrices of a missing data pattern, y = So*yo + Sm*ym,
...
% An all-observed or all-missing pattern gives the corresponding matrix Tn x 0. The body
% is that of ssm.select_obs in statespace-toolkit, line for line, apart from the error
% identifier.
%
% See:
...
if ~isnumeric(Y) || ~ismatrix(Y) || isempty(Y)
    error('bvar:util:select_obs:badY', 'Y must be a nonempty T x n numeric matrix');
end
[T, n] = size(Y);
