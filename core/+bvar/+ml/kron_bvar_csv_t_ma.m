...
for isim = 1:nsims
    ...
    if ~bugcompat
            % corrected: integrate over the stored psi draws, the pattern
            % bvar.ml.kron_bvar_csv_ma follows
...
lpost = log(mean(exp(store_lpost-repmat(tmpmax,nsims,1)))) + tmpmax;
    ...
    % rho and psi ordinates: reduced run; (h, lam, rho) continue from the
    % last stored draws, psi from the final chain value
    % (= store_theta(nsims,1) in both modes), psihat from the estimation run
store_lpost1 = store_lpost;
store_lpost = zeros(nsims2,2); % [log density of rho, log density of psi]
...
for isim = 1:nsims2
    ...
    h = bvar.sv.csv_armh(s2_h,rho,sigh2,h,n);
        ...
        % sample lam [no (1+psi^2) first-observation correction, mirroring
        % the estimation sampler]
    s2_lam = sum(tmp.^2,2)./exp(h);
    lam = 1./gamrnd((n+nu)/2,2./(s2_lam+nu));

        % sample rho [reduced-run bound .999; the estimation run uses .99]
