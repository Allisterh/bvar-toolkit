%% GOLDEN-RUN VARIANT PATCH (2026-09-17): identical to legacy/sp_code/TVPVAR.m except that
%% the plotting block at the end is replaced by a numeric capture, which prints the
%% posterior means the script already computes and saves them to golden_tvpvar.mat. The computation is untouched.
% % =======================================================================
% % TVP-VAR example
...
fprintf('Omega11hat diagonal: %.6g %.6g %.6g %.6g\n', diag(Omega11hat));
fprintf('Omega22hat: mean %.6g, min %.6g, max %.6g\n', mean(Omega22hat), min(Omega22hat), max(Omega22hat));
fprintf('norm(betahat): %.6g\n', norm(betahat));
fprintf('GDP equation, own lag at the first and last period: %.6g, %.6g\n', gamhat1(1,2), gamhat1(end,2));
fprintf('interest rate equation, own lag at the first and last period: %.6g, %.6g\n', gamhat4(1,5), gamhat4(end,5));
save('golden_tvpvar.mat','betahat','Omega11hat','Omega22hat','gamhat1','gamhat2','gamhat3','gamhat4');
