...
VAR in one block under those restrictions, hard or soft; `test_missing_var` checks the conditional
mean and covariance against dense conditioning on the observed values and the restrictions, of
either kind), `bvar.sv.csv_armh_block` (the accept-reject Metropolis-Hastings step of
`bvar.sv.csv_armh` applied to one block of the path at a time, which keeps a high acceptance rate
on a long sample with a few very large errors; `test_csv_armh_block` checks that chains with
blocks of length 1 and 2 and with the path as one block match a grid posterior, at two envelope
constants), `bvar.samplers.var_coef_joint` (all VAR coefficients in one draw under an independent
normal prior, with a weight per period; `test_var_coef_joint` checks the mean and covariance of
its draws against dense algebra) and `bvar.models.mfvar_csv` (the sampler of ex14;
`test_mfvar_csv` also pins its prior variances). None of these is retrofitted into a legacy body:
they exist for new code, and the legacy spellings they generalize stay as they are.
...
- **`sign_restrict` / `sign_assign`**: two correct algorithms for the same job, with very
  ...
  **They also differ on an empty ranking set**, and this difference is unintended:
  ...
  and a `0 x n` `Rineq`. Both headers now say so. `examples/ex12_dynamic_sign_restrictions.m`
...
`replications/chan_eisenstat2018_jae_mltvpsv/run_all.m` runs the estimation of all ten models
of `main_tvpsv.m`, with the constants of the ten model scripts in `preset.m`, each cited to its
legacy lines. It names the models as the paper does: models 6 and 7, which the scripts print as
VAR-SV and VAR, are CVAR-SV and CVAR, and models 9 and 10, RS-VAR-1 and RS-VAR-2 in the
driver's comment, are RS-VAR-R1 and RS-VAR-R2. The legacy spellings are accepted too.

`test_mltvpsv_equivalence` runs the ten scripts from tempdir copies beside `run_all` and
requires identical stored draws, script-tail summaries, design matrices, log prior at the last
draw and terminal rng state at 22 settings: every model at the legacy default p = 2 (r = 2),
p = 1 and p = 4 on a model of each design, and the three RS models at r = 3 and r = 12. It takes
12 seconds. The package reads its data with `xlsread` and a range, so CI skips it;
`test_mltvpsv_options` runs `run_all` on generated data on CI (every model, the sizes of the
...
substitutions for the legacy copies), and the timing displays are not reproduced. Three quirks
are reproduced as published:

- `TVP_R3_SV.m` line 100 draws mu_0 with the current mu_0 as its prior mean; the script defines
  the prior mean `amu = 0` at line 15 and never uses it.
- In `VAR_RS_R2.m` a regime with no observations redraws the common error variances from their
  prior inside the regime loop (line 77), so the coefficient draws of the later regimes in that
  sweep use the prior draw. The variances are drawn from their conditional after the loop.
- Every RS script starts the filter from p(s_1) = 1/3, whatever r is. The filter normalizes each
  step, so the draws do not depend on it.

At r = 12 and p = 1 all three RS models visit empty regimes (in 40 sweeps, RS-VAR-R1 10 times and
RS-VAR-R2 153; at r = 6 neither did), so the prior-draw branches, the RS-VAR-R2 quirk among
them, are compared bitwise.

In a scratch mirror, each of these changes makes the test fail: 1e-9 on the prior variance, on
the scale of the log-volatility innovations, or on a mixture mean in `bvar.sv.ksc_rw_h0`; the
TVP-SV mean solved with backslash; and either quirk corrected. The starting transition matrix
