...
| `bvar.forecast` | Forecasts from a chain: `iterate` runs one draw forward and scores it, `predictive` returns the mean and standard deviation of the h-step predictive distribution of every draw, `tables` accumulates RMSFEs and log predictive likelihoods, and `realtime_loaddata` assembles a real-time data vintage. |
