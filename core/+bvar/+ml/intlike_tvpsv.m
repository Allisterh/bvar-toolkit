% bvar.ml.intlike_tvpsv - log integrated likelihood of a TVP regression with
% random-walk stochastic volatility, by importance sampling of the log-volatilities
% h. The states theta_t are integrated out analytically for each draw of h; the
% importance density of h is Gaussian at the mode of its marginal density, found by
% an EM algorithm, with precision from the negative Hessian there.
%
%   [llike, store_llike] = bvar.ml.intlike_tvpsv(Y, Sigtheta, Sigh, bigX, h0, theta0)
%   [llike, store_llike] = bvar.ml.intlike_tvpsv(..., R)
%
...
%   h0, theta0 : n x 1 and k x 1 initial states
%   R        : number of importance draws. Omitted, R = 10, and when the variance
%              of the mean log weight exceeds 1 the draws are increased to
%              R*(floor of that variance + 1).
%   store_llike : the log importance weights
%
% The EM algorithm stops when no log-volatility moves by more than .1, or after 100
% iterations; its Newton step stops at the same tolerance or after 1000 steps. If
% the EM reaches 100 iterations, the importance density is centered at h0 instead.
%
% See:
...
while e_h > .1 && countout < 100
    ...
    CKl = chol(Ktheta,'lower');
    thetahat = CKl'\(CKl\dtheta);
    CKtheta = CKl';
...
Z = XinvSig'*(CKtheta\(CKtheta'\bigX'));
HH = -.5*Z'.*(eye(n*T)-Z);
Kh = -(HQ+HH);
Cg = chol(Kh,'lower');
...
% the log likelihood given the log-volatilities h (T x n), the states integrated out
[T,n] = size(h);
k = size(Sigtheta,1);
Htheta = speye(T*k) - sparse(k+1:T*k,1:(T-1)*k,ones((T-1)*k,1),T*k,T*k);
...
CKd = chol(Ktheta,'lower');

llike = -T*n/2*log(2*pi) - T/2*sum(log(Sigtheta)) - .5*sum(sum(h)) ...
    - sum(log(diag(CKd))) - .5*(Y'*invSig*Y ...
    + alptheta'*HinvSH*alptheta - dtheta'*(CKd'\(CKd\dtheta)));
end
