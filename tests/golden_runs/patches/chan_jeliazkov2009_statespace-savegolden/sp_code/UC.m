%% GOLDEN-RUN VARIANT PATCH (2026-09-17): identical to legacy/sp_code/UC.m except that
%% the plotting block at the end is replaced by a numeric capture, which prints the
%% posterior means the script already computes and saves them to golden_uc.mat. The computation is untouched.
% % =======================================================================
...
omega2tau_hat = mean(store_omega2tau);
omega2h_hat = mean(store_omega2h);
fprintf('posterior mean omega2tau: %.6g, omega2h: %.6g\n', omega2tau_hat, omega2h_hat);
fprintf('trend inflation tauhat, first and last period: %.6g, %.6g\n', tauhat(1), tauhat(end));
fprintf('mean tauhat: %.6g, mean exp(hhat/2): %.6g\n', mean(tauhat), mean(exp(hhat/2)));
save('golden_uc.mat','tauhat','hhat','omega2tau_hat','omega2h_hat');
