...
% exactly once, or as many times as an optional third column gives. Only the
% solves change; a legacy draw line or log determinant that factors K again
% obtains the same factor.
...
ml_tvpsv_theta = {'thetahat = Ktheta\dtheta;', ...
    'CKtheta = chol(Ktheta,''lower''); thetahat = (CKtheta'')\(CKtheta\dtheta);'};
ml_tvpsv_rs = @(rhs) {['theta_hat = Ktheta\(atheta./Vtheta + ' rhs ');'], ...
    ['CKtheta = chol(Ktheta,''lower''); theta_hat = (CKtheta'')\(CKtheta\(atheta./Vtheta + ' rhs '));']};
% the precision of a Gaussian importance density, from the factor its draws use
ml_pre = @(name, arg, dim) {[name 'pre = cov(' arg ')\speye(' dim ');'], ...
    [name 'pre = (' name 'std'')\(' name 'std\speye(' dim '));']};
...
subs = {
    ...
    % the ten model scripts of chan_eisenstat2018_jae_mltvpsv/run_all.m; the
    % diagonal precisions of the initial states keep their backslash, which divides
    'chan_eisenstat2018_jae_mltvpsv/legacy/TVPSV.m',  ml_tvpsv_theta
    'chan_eisenstat2018_jae_mltvpsv/legacy/TVP.m',    ml_tvpsv_theta
    'chan_eisenstat2018_jae_mltvpsv/legacy/TVP_R1_SV.m', ...
        {'gamhat = Kgam\(HinvSH*alpgam + WinvSig*(Y-Xtilde*beta));', ...
         'CKgam = chol(Kgam,''lower''); gamhat = (CKgam'')\(CKgam\(HinvSH*alpgam + WinvSig*(Y-Xtilde*beta)));'; ...
         'betahat = Kbeta\(beta0./Vbeta + XtildeinvSig*(Y-W*gam));', ...
         'CKbeta = chol(Kbeta,''lower''); betahat = (CKbeta'')\(CKbeta\(beta0./Vbeta + XtildeinvSig*(Y-W*gam)));'}
    'chan_eisenstat2018_jae_mltvpsv/legacy/TVP_R2_SV.m', ...
        {'betahat = Kbeta\(HinvSH*alpbeta + XinvSig*(Y-W*gam));', ...
         'CKbeta = chol(Kbeta,''lower''); betahat = (CKbeta'')\(CKbeta\(HinvSH*alpbeta + XinvSig*(Y-W*gam)));'; ...
         'gamhat = Kgam\(gam0./Vgam + WinvSig*(Y-Xtilde*beta));', ...
         'CKgam = chol(Kgam,''lower''); gamhat = (CKgam'')\(CKgam\(gam0./Vgam + WinvSig*(Y-Xtilde*beta)));'}
    'chan_eisenstat2018_jae_mltvpsv/legacy/TVP_R3_SV.m', ...
        {'muhat = Kmu\(HinvSH_mu*alpmu + invSig*(Y-Ztilde*beta-W*gam));', ...
         'CKmu = chol(Kmu,''lower''); muhat = (CKmu'')\(CKmu\(HinvSH_mu*alpmu + invSig*(Y-Ztilde*beta-W*gam)));'; ...
         'beta_hat = Kbeta\([beta0./Vbeta;gam0./Vgam] + ZinvSig*(Y-mu));', ...
         'CKbeta = chol(Kbeta,''lower''); beta_hat = (CKbeta'')\(CKbeta\([beta0./Vbeta;gam0./Vgam] + ZinvSig*(Y-mu)));'}
    'chan_eisenstat2018_jae_mltvpsv/legacy/VAR_SV.m', ...
        {'thetahat = Ktheta\(atheta./Vtheta + XinvSig*Y);', ...
         'CKtheta = chol(Ktheta,''lower''); thetahat = (CKtheta'')\(CKtheta\(atheta./Vtheta + XinvSig*Y));'}
    'chan_eisenstat2018_jae_mltvpsv/legacy/VAR.m', ...
        {'theta_hat = Ktheta\(atheta./Vtheta + XinvSig*Y);', ...
         'CKtheta = chol(Ktheta,''lower''); theta_hat = (CKtheta'')\(CKtheta\(atheta./Vtheta + XinvSig*Y));'}
    'chan_eisenstat2018_jae_mltvpsv/legacy/VAR_RS.m',    ml_tvpsv_rs('XiSig*Yi')
    'chan_eisenstat2018_jae_mltvpsv/legacy/VAR_RS_R1.m', ml_tvpsv_rs('XiSig*Y')
    'chan_eisenstat2018_jae_mltvpsv/legacy/VAR_RS_R2.m', ml_tvpsv_rs('XiSig*Yi')
    % its integrated likelihoods and marginal likelihoods: bvar.ml.intlike_tvp,
    % intlike_tvpsv, intlike_cvarsv and the ten bvar.ml.mltvpsv_* routines
    'chan_eisenstat2018_jae_mltvpsv/legacy/intlike_tvp.m', ...
        {'dtheta = XinvSig*Y + HinvSH*alptheta;', ...
         'dtheta = XinvSig*Y + HinvSH*alptheta; CKtheta = chol(Ktheta,''lower'');'; ...
         'sum(log(diag(chol(Ktheta))))', 'sum(log(diag(CKtheta)))'; ...
         'dtheta''*(Ktheta\dtheta)', 'dtheta''*((CKtheta'')\(CKtheta\dtheta))'}
    'chan_eisenstat2018_jae_mltvpsv/legacy/intlike_tvpsv.m', ...
        {'thetahat = Ktheta\dtheta;', ...
         'CKl = chol(Ktheta,''lower''); thetahat = (CKl'')\(CKl\dtheta);', 2; ...
         'Z = XinvSig''*(Ktheta\bigX'');', 'Z = XinvSig''*(CKtheta\(CKtheta''\bigX''));', 1; ...
         'dtheta = XinvSig*Y + HinvSH*alptheta;', ...
         'dtheta = XinvSig*Y + HinvSH*alptheta; CKd = chol(Ktheta,''lower'');', 1; ...
         'sum(log(diag(chol(Ktheta))))', 'sum(log(diag(CKd)))', 1; ...
         'dtheta''*(Ktheta\dtheta)', 'dtheta''*((CKd'')\(CKd\dtheta))', 1}
    'chan_eisenstat2018_jae_mltvpsv/legacy/intlike_varsv.m', ...
        {'newht = ht - Gh\gh;', 'CKh = chol(-Gh,''lower''); newht = ht + (CKh'')\(CKh\gh);'}
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_tvpsv.m', ...
        [ml_pre('theta0', 'store_theta0', 'k'); ml_pre('h0', 'store_h0', 'n')]
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_tvp.m',    ml_pre('theta0', 'store_theta0', 'k')
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_tvp_r1_sv.m', ...
        [ml_pre('beta', 'store_beta', 'kbeta'); ml_pre('gam0', 'store_gam0', 'kgam'); ...
         ml_pre('h0', 'store_h0', 'n')]
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_tvp_r2_sv.m', ...
        [ml_pre('gam', 'store_gam', 'kgam'); ml_pre('beta0', 'store_beta0', 'kbeta'); ...
         ml_pre('h0', 'store_h0', 'n')]
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_tvp_r3_sv.m', ...
        [ml_pre('beta', '[store_beta store_gam]', 'kgam+kbeta'); ml_pre('mu0', 'store_mu0', 'n'); ...
         ml_pre('h0', 'store_h0', 'n')]
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_varsv.m', ...
        [ml_pre('theta', 'store_theta', 'q'); ml_pre('h0', 'store_h0', 'n')]
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_var.m',       ml_pre('theta', 'store_theta', 'k')
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_var_rs.m',    ml_pre('theta', 'store_theta', 'k*r')
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_var_rs_r1.m', ml_pre('theta', 'store_theta', 'k')
    'chan_eisenstat2018_jae_mltvpsv/legacy/ml_var_rs_r2.m', ml_pre('theta', 'store_theta', 'k*r')
    };
...
for ks = 1:size(pairs,1)
    ...
    assert(numel(strfind(txt, pairs{ks,1})) == expected, ...
        'one_factor_patch: expected %d of %s in %s', expected, pairs{ks,1}, rel);
