% bvar.ml.mltvpsv_rs - log marginal likelihood of the RS-VAR model of Chan and Eisenstat
...
%
%   [lml, lmlstd, out] = bvar.ml.mltvpsv_rs(Y, store_theta, store_Sig, store_P, ...
...
function [ml, mlstd, out] = mltvpsv_rs(Y,store_theta,store_Sig,store_P,prior,bigX,M,bugcompat)
if nargin < 8
    bugcompat = false;
end
M = 20*ceil(M/20);
...
for isim = 1:M
    ...
    llike = bvar.ml.intlike_rsvar(shortY,bigX,reshape(theta,k,r),reshape(Sig,n,r),P,p1);
