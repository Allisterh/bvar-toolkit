...
%              (n = numel(mu)), the last r are zero-mean and share the same phi
%              and sig2 draws
...
%   phi_bnd  : the phi candidate is accepted only if |phic| < phi_bnd; default
%              .999, the OISV canonical value, while ml_varsv uses .998
%   sig2     : (n+r) x 1 innovation variances
%   flag_phi : (n+r) x 1, 1 where the phi candidate was accepted
%
% Trap: the mu block is gated on `if mu~=0`, which runs only when EVERY element
% of mu is nonzero; a single exact zero silently skips the mu draw for all n
% series.
%
